...
Base.:/(x::Form{D,R}, a) where {D,R} = Form{D,R}(x.elts / a)
Base.://(x::Form{D,R}, a) where {D,R} = Form{D,R}(x.elts // a)
Base.cld(x::Form{D,R}, a) where {D,R} = Form{D,R}(map(b -> cld(b, a), x.elts))
Base.div(x::Form{D,R}, a) where {D,R} = Form{D,R}(map(b -> div(b, a), x.elts))
Base.fld(x::Form{D,R}, a) where {D,R} = Form{D,R}(map(b -> fld(b, a), x.elts))
Base.mod(x::Form{D,R}, a) where {D,R} = Form{D,R}(map(b -> mod(b, a), x.elts))
Base.rem(x::Form{D,R}, a) where {D,R} = Form{D,R}(map(b -> rem(b, a), x.elts))
