...
@testset "Create tensor forms" begin
    for iter in 1:100
        D1 = rand(0:4)
        D2 = rand(0:4)
        R1 = rand(0:D1)
        ...
        x = rand(DoubleForm{D1,D2,R1,R2,T})

        @test DoubleForm{D1,D2,R1,R2,Float64}(x) == DoubleForm{D1,D2,R1,R2,Float64}(x.form)
    end
end

@testset "Comparing tensor forms" begin
    for iter in 1:100
        D1 = rand(0:4)
        D2 = rand(0:4)
        R1 = rand(0:D1)
        ...
        x = rand(DoubleForm{D1,D2,R1,R2,T})
        y = rand(DoubleForm{D1,D2,R1,R2,T})
...
@testset "Double forms as collections" begin
    for iter in 1:100
        ...
        for n in 1:N
            ...
            inds1 = Forms.lin2lst(Val(D1), Val(R1), n1)::SVector{R1,Int}
            inds2 = Forms.lin2lst(Val(D2), Val(R2), n2)::SVector{R2,Int}
...
@testset "Vector space operations on forms" begin
    ...
    for iter in 1:100
        ...
        T = Rational{Int64}
        n = zero(DoubleForm{D1,D2,R1,R2,T})
        x = rand(DoubleForm{D1,D2,R1,R2,T})
        y = rand(DoubleForm{D1,D2,R1,R2,T})
        z = rand(DoubleForm{D1,D2,R1,R2,T})
        a = rand(T)
        b = rand(T)
...
@testset "Double form algebra" begin
    ...
    for iter in 1:100
        ...
        T = Rational{Int64}
        e = one(DoubleForm{D1,D2,0,0,T})
        x = rand(DoubleForm{D1,D2,R1x,R2x,T})
        y = rand(DoubleForm{D1,D2,R1y,R2y,T})
        y2 = rand(DoubleForm{D1,D2,R1y,R2y,T})
        a = rand(T)
        b = rand(T)
        ...
        # units
        if D1 == 2 && D2 == 2 && R1x == 0 && R1y == 0 && R2x == 0 && R2y == 0
            e00 = unit(DoubleForm{D1,D2,0,0,T}, ())
            e10 = unit(DoubleForm{D1,D2,1,0,T}, (1,))
            e20 = unit(DoubleForm{D1,D2,1,0,T}, (2,))
            e30 = unit(DoubleForm{D1,D2,2,0,T}, (1, 2))
            e01 = unit(DoubleForm{D1,D2,0,1,T}, (1,))
            e11 = unit(DoubleForm{D1,D2,1,1,T}, (1, 1))
            e21 = unit(DoubleForm{D1,D2,1,1,T}, (2, 1))
            e31 = unit(DoubleForm{D1,D2,2,1,T}, (1, 2, 1))
            e02 = unit(DoubleForm{D1,D2,0,1,T}, (2,))
            e12 = unit(DoubleForm{D1,D2,1,1,T}, (1, 2))
            e22 = unit(DoubleForm{D1,D2,1,1,T}, (2, 2))
            e32 = unit(DoubleForm{D1,D2,2,1,T}, (1, 2, 2))
            e03 = unit(DoubleForm{D1,D2,0,2,T}, (1, 2))
            e13 = unit(DoubleForm{D1,D2,1,2,T}, (1, 1, 2))
            e23 = unit(DoubleForm{D1,D2,1,2,T}, (2, 1, 2))
            e33 = unit(DoubleForm{D1,D2,2,2,T}, (1, 2, 1, 2))
        ...
        @test swap(x) isa DoubleForm{D2,D1,R2x,R1x,T}
        @test swap(swap(x)) === x
        ...
        @test ⋆⋆x === bitsign(R1x * (D1 - R1x)) * bitsign(R2x * (D2 - R2x)) * x
        @test inv(⋆)(⋆x) === x
        @test ⋆inv(⋆)(x) === x
        @test ⋆⋆ ⋆ ⋆x === x
        @test a * ⋆x === ⋆(a * x)
        ...
        if R1x + R1y <= D1 && R2x + R2y <= D2
            (x ∧ y)::DoubleForm{D1,D2,R1x + R1y,R2x + R2y}
        end

        for R1z in 0:D1, R2z in 0:D2
            z = rand(DoubleForm{D1,D2,R1z,R2z,T})
            if R1x + R1y + R1z <= D1 && R2x + R2y + R2z <= D2
                @test (x ∧ y) ∧ z === x ∧ (y ∧ z)
                @test x ∧ y ∧ z === (x ∧ y) ∧ z
                @test ∧(x, y, z) === (x ∧ y) ∧ z
        ...
        @test ∨(x) === x
        R1vee = D1 - ((D1 - R1x) + (D1 - R1y))
        R2vee = D2 - ((D2 - R2x) + (D2 - R2y))
        @test R1vee == R1x + R1y - D1
        @test R2vee == R2x + R2y - D2
        if 0 <= R1vee <= D1 && 0 <= R2vee <= D2
            (x ∨ y)::DoubleForm{D1,D2,R1vee,R2vee}
            @test ⋆(x ∨ y) === ⋆x ∧ ⋆y
        end

        for R1z in 0:D1, R2z in 0:D2
            z = rand(DoubleForm{D1,D2,R1z,R2z,T})
            R1veez = D1 - ((D1 - R1x) + (D1 - R1y) + (D1 - R1z))
            R2veez = D2 - ((D2 - R2x) + (D2 - R2y) + (D2 - R2z))
            if 0 <= R1veez <= D1 && 0 <= R2veez <= D2
                (x ∨ y ∨ z)::DoubleForm{D1,D2,R1veez,R2veez}
                @test ⋆(x ∨ y ∨ z) === ⋆x ∧ ⋆y ∧ ⋆z
                @test x ∨ y ∨ z === (x ∨ y) ∨ z
                @test ∨(x, y, z) === (x ∨ y) ∨ z
                # @test ∨(SVector{0,typeof(x)}()) === ⋆one(x)
        ...
        @test R1dot == R1x - R1y
        @test R2dot == R2x - R2y
        if 0 <= R1dot <= D1 && 0 <= R2dot <= D2
            (x ⋅ y)::DoubleForm{D1,D2,R1dot,R2dot}
            @test x ⋅ y === x ∨ ⋆y
        end
        ...
        xs = rand(DoubleForm{D1,D2,R1x,R2x,SVector{4,T}})
        xs2 = rand(DoubleForm{D1,D2,R1x,R2x,SVector{4,T}})
        norm2(xs)::T
        @test norm2(xs) >= 0
        ...
        if 0 <= R1cross && 0 <= R2cross
            (x × y)::DoubleForm{D1,D2,R1cross,R2cross}
            @test x × y === ⋆(x ∧ y)
        end
    end
end
